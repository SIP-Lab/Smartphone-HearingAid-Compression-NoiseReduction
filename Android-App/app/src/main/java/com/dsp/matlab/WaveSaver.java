			...
			if(outputFrame != null){
				...
				if(debugData){
					try {
						...
						debugWriter.write(("VAD:" + Double.toString(outputFrame.getVAD())+"\n" + "SPL:" + Double.toString(outputFrame.getSPL())+"\n" + "Filter " + Double.toString(outputFrame.getFilter())+ " Applied" + "\n").getBytes());
					} catch (IOException e) {
						e.printStackTrace();
					}
				}
				...
				if(framecount == 0){
					...
					main.updateVAD(outputFrame.getVAD());
					main.updateSPL(outputFrame.getSPL());
					main.updateFilter(outputFrame.getFilter());
				} else {
					elapsed += outputFrame.getElapsed();
				}
			}
